...
func (s *InboundService) syncClientRateLimit(tx *gorm.DB, email string, egressBps, ingressBps int64) error {
	...
	if egressBps <= 0 && ingressBps <= 0 {
		return tx.Where("email = ?", email).Delete(&model.ClientRateLimit{}).Error
	}

	now := time.Now().UnixMilli()
	var rl model.ClientRateLimit
	err := tx.Where("email = ?", email).First(&rl).Error
	if err != nil {
...
func (s *InboundService) syncClientRateLimits(tx *gorm.DB, clients []model.Client) error {
...
func (s *InboundService) syncClientRateLimitChanges(tx *gorm.DB, oldClients, newClients []model.Client) error {
	newEmails := make(map[string]string, len(newClients))
	for _, client := range newClients {
		email := strings.TrimSpace(client.Email)
		if email == "" {
			continue
		}
		newEmails[strings.ToLower(email)] = email
	}
	...
	for _, client := range oldClients {
		...
		if _, ok := newEmails[strings.ToLower(email)]; !ok {
			if err := tx.Where("email = ?", email).Delete(&model.ClientRateLimit{}).Error; err != nil {
				return err
			}
		}
	}

	return s.syncClientRateLimits(tx, newClients)
}

func (s *InboundService) clientHasRateLimit(client model.Client) bool {
	return client.EgressBps > 0 || client.IngressBps > 0
}

func (s *InboundService) clientsRequireRateLimitRestart(oldClients, newClients []model.Client) bool {
	oldMap := make(map[string]model.Client, len(oldClients))
	newMap := make(map[string]model.Client, len(newClients))
	...
	for email, oldClient := range oldMap {
		newClient, ok := newMap[email]
		if !ok {
			if s.clientHasRateLimit(oldClient) {
				return true
			}
			continue
		}
		if oldClient.EgressBps != newClient.EgressBps || oldClient.IngressBps != newClient.IngressBps {
			return true
		}
	}

	for email, newClient := range newMap {
		if _, ok := oldMap[email]; !ok && s.clientHasRateLimit(newClient) {
			return true
		}
	}
...
func (s *InboundService) AddInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	if err == nil {
		...
		if err = s.syncClientRateLimits(tx, clients); err != nil {
			return inbound, false, err
		}
	} else {
		return inbound, false, err
...
func (s *InboundService) DelInbound(id int) (bool, error) {
	...
	if s.clientsRequireRateLimitRestart(clients, nil) {
		needRestart = true
	}
	for _, client := range clients {
		...
		if err := db.Where("email = ?", client.Email).Delete(&model.ClientRateLimit{}).Error; err != nil {
			return false, err
		}
	}
...
func (s *InboundService) UpdateInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	rateLimitRestart := s.clientsRequireRateLimitRestart(oldClients, newClients)
...
func (s *InboundService) updateClientTraffics(tx *gorm.DB, oldInbound *model.Inbound, newInbound *model.Inbound) error {
	...
	return s.syncClientRateLimitChanges(tx, oldClients, newClients)
}
...
func (s *InboundService) AddInboundClient(data *model.Inbound) (bool, error) {
	...
	s.xrayApi.Close()
	if s.clientsRequireRateLimitRestart(nil, clients) {
		needRestart = true
	}
...
func (s *InboundService) DelInboundClient(inboundId int, clientId string) (bool, error) {
	...
	if len(email) > 0 {
		...
		if err := db.Model(&model.ClientRateLimit{}).Where("email = ?", email).Count(&rateLimitCount).Error; err != nil {
			return false, err
		}
		if rateLimitCount > 0 {
			needRestart = true
		...
		if err := db.Where("email = ?", email).Delete(&model.ClientRateLimit{}).Error; err != nil {
			return false, err
		}
...
func (s *InboundService) UpdateInboundClient(data *model.Inbound, clientId string) (bool, error) {
	...
	if err = s.syncClientRateLimitChanges(tx, oldClients[clientIndex:clientIndex+1], clients[:1]); err != nil {
		return false, err
	}
	needRestart := false
	if s.clientsRequireRateLimitRestart(oldClients[clientIndex:clientIndex+1], clients[:1]) {
		needRestart = true
	}
	if len(oldEmail) > 0 {
		s.xrayApi.Init(p.GetAPIPort())
