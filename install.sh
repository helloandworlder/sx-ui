...
GITHUB_OWNER="${GITHUB_OWNER:-helloandworlder}"
...
setup_ip_certificate() {
    ...
    echo -e "${yellow}acme.sh will automatically renew and reload sx-ui before expiry.${plain}"
    return 0
}
...
# ── sx-ui: Configure node type and API key ────────────────────────────
# Supports both interactive and non-interactive modes.
# Non-interactive: SX_NODE_TYPE=residential SX_API_KEY=xxx ./install.sh
configure_sxui_node() {
    local node_type="${SX_NODE_TYPE:-}"
    local api_key="${SX_API_KEY:-}"

    echo ""
    echo -e "${green}═══════════════════════════════════════════${plain}"
    echo -e "${green}     sx-ui Node Configuration               ${plain}"
    echo -e "${green}═══════════════════════════════════════════${plain}"

    # Node type
    if [[ -z "$node_type" ]]; then
        echo -e "${yellow}Select node type:${plain}"
        echo -e "  ${blue}[1]${plain} Residential (家宽) — IP pool, exit-IP binding"
        echo -e "  ${blue}[2]${plain} Dedicated  (专线) — Fixed outbound, relay acceleration"
        while true; do
            read -rp "Enter choice [1/2]: " choice
            case "$choice" in
                1) node_type="residential"; break ;;
                2) node_type="dedicated"; break ;;
    ...
    # API key
    if [[ -z "$api_key" ]]; then
        api_key=$(cat /proc/sys/kernel/random/uuid 2>/dev/null || gen_random_string 36)
    fi

    # GeoIP block CN (residential only)
    local geoip_block="false"
    if [[ "$node_type" == "residential" ]]; then
        if [[ -z "${SX_GEOIP_BLOCK_CN:-}" ]]; then
            read -rp "Block mainland China IPs from connecting? [y/N]: " block_choice
            if [[ "${block_choice}" == "y" || "${block_choice}" == "Y" ]]; then
                geoip_block="true"
            fi
        else
            geoip_block="${SX_GEOIP_BLOCK_CN}"
        fi
    fi
...
config_after_install() {
    ...
    # ── sx-ui extensions: node type & API key ──────────────────────────
    configure_sxui_node
}
...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "${GITHUB_RELEASE_API}" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
            tag_version=$(curl -4 -Ls "${GITHUB_RELEASE_API}" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
                echo -e "${red}Failed to fetch sx-ui version, it may be due to GitHub API restrictions, please try it later${plain}"
                exit 1
            fi
        fi
        echo -e "Got sx-ui latest version: ${tag_version}, beginning the installation..."
        curl -4fLRo "${temp_dir}/${archive_name}" "${GITHUB_RELEASE_DOWNLOAD_BASE}/${tag_version}/${archive_name}"
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Downloading sx-ui failed, please be sure that your server can access GitHub ${plain}"
            exit 1
        fi
    else
        tag_version=$1
        ...
        url="${GITHUB_RELEASE_DOWNLOAD_BASE}/${tag_version}/${archive_name}"
        echo -e "Beginning to install sx-ui $1"
        curl -4fLRo "${temp_dir}/${archive_name}" "${url}"
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download sx-ui $1 failed, please check if the version exists ${plain}"
            exit 1
        fi
    fi
    ...
    rm -rf "${xui_folder}"
    mv "${temp_dir}/${bundle_dir}" "${xui_folder}"
    rm -rf "${temp_dir}"
    ...
    if [ -d "/etc/.git" ]; then
        if [ -f "/etc/.gitignore" ]; then
            if ! grep -q "sx-ui/" "/etc/.gitignore"; then
                echo "" >> "/etc/.gitignore"
                echo "sx-ui/" >> "/etc/.gitignore"
                echo -e "${green}Added sx-ui data path to /etc/.gitignore for etckeeper${plain}"
            fi
        else
            echo "sx-ui/" > "/etc/.gitignore"
            echo -e "${green}Created /etc/.gitignore and added sx-ui data path for etckeeper${plain}"
        fi
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        chmod +x "/etc/init.d/${xui_service_name}"
        rc-update add "${xui_service_name}"
    ...
    echo -e "${green}sx-ui ${tag_version}${plain} installation finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
