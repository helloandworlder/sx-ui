...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz ${GITHUB_RELEASE_DOWNLOAD_BASE}/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="${GITHUB_RELEASE_DOWNLOAD_BASE}/${tag_version}/x-ui-linux-$(arch).tar.gz"
